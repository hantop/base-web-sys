    ...
    computed: {
      ...
      data2 () {
        ...
        return [{label: '贷款金额', value: data.projectAmount}, {label: '期限', value: data.projectTerm + this.$dict.findDictionaryName(data.projectTermUnit)}, {label: '最短收息期', value: data.minday + this.$dict.findDictionaryName(data.mindayUnit)}, {
          label: '还款方式', value: this.$dict.findDictionaryName(data.paymentplanId)}, {label: '费用方案', value: data.feeplanId === 1 ? '默认方案' : '管理费方案'}, {
            label: '费率', value: data.feeplanId === 1 ? data.projectRate + '%/' + this.$dict.findDictionaryName(data.projectrateUnit) : '利率(月)' + data.projectRate + '%' + '; 管理费率：' + data.managementRate + '%/' + this.$dict.findDictionaryName(data.projectrateUnit)}, {
