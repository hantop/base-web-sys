  ...
  <div v-loading.fullscreen="loading" :class="{'g-listpage__style' : !isDisabled}" >
    ...
    <el-table :data="guaranteeData" border class="g-table"
     ...
     row-key="id" ref="table">
      ...
      <el-table-column prop="guaranteeType" label="担保方式" width="100">
      </el-table-column>
      <el-table-column prop="name" label="担保人\担保公司名称" width="160">
      </el-table-column>
      <el-table-column prop="legalCardNum" label="身份证" width="200">
      </el-table-column>
      ...
      <el-table-column prop="guaranteeState" label="状态" v-if="!isDisabled">
    ...
    <el-dialog :title="guaranteeType==='0'? '自然人担保' : '公司担保'"
...
export default {
  components: {guaranteePerson,guaranteeCorporate},
  data() {
    return {
      rules: Rules,
      ...
      guaranteeController
        ...
        .then(res => {
          if (res.status === 200) {
            ...
            if (data) {
              data.forEach(v => {
                v.guaranteeType = v.type === "0" ? '个人担保' : '公司担保'
