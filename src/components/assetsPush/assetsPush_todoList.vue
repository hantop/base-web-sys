<template>
	<div>
		...
		<el-table :data="todoList" border class="g-table" ref="todoList_table">
			...
			<el-table-column prop="orderStatus" label="募集状态">
			</el-table-column>
			<el-table-column prop="checkPassTime" label="审批时间" width="180" :formatter="(row) => $formatDate(row.checkPassTime, 'yyyy-MM-dd hh:mm:ss')">
			</el-table-column>
			<el-table-column prop="submitApplyTime" label="申请时间" width="180" :formatter="(row) => $formatDate(row.submitApplyTime, 'yyyy-MM-dd hh:mm:ss')">
			</el-table-column>
